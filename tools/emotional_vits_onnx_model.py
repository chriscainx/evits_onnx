...
import commons
import emotional_vits_onnx_modules as modules
import attentions
...
class TextEncoder(nn.Module):
    ...
    def forward(self, x, x_lengths, emo):
        ...
        x = x + self.emo_proj(emo.unsqueeze(0).unsqueeze(1))
...
class SynthesizerTrn(nn.Module):
    ...
    def forward(self, x, x_lengths, emo=None):
        noise_scale = 0.667
        length_scale = 1
        noise_scale_w = 0.8
        max_len = None
        torch.onnx.export(
            ...
            (x, x_lengths, emo),
            "Pieces_enc_p.onnx",
            input_names=["x", "x_lengths", "emotion"],
        ...
        torch.onnx.export(
            ...
            output_names=["logw"],
            dynamic_axes={"x": [2], "x_mask": [2], "zin": [0, 2], "logw": [2]},
            operator_export_type=OperatorExportTypes.ONNX_FALLTHROUGH,
        )
        logw = self.dp(x, x_mask, zinput)
